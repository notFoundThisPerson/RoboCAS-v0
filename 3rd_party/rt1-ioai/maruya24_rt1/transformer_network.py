...
from maruya24_rt1.transformer import Transformer, TF_MultiHeadAttention
...
class TransformerNetwork(nn.Module):
    ...
    def __init__(
            ...
            vocab_size: int = 256,
            # Dimensionality of tokens from the output layer. This is also dimensionality of tokens from the input layer.
            token_embedding_size: int = 512,
            # RT1ImageTokenizer outputs(=context_image_tokens) has embedding dimension of token_embedding_size. This will finally be utilized in 1x1 Conv in EfficientNetEncoder class.
            language_embedding_size: int = 512,  # embedding size for language embedding and Film layers
            num_layers: int = 1,
            layer_size: int = 4096,
            # This corresponds to key_dim which is the size of each attention head for query, key and values.
            num_heads: int = 8,
            feed_forward_size: int = 512,
            # This corresponds to d_model which is embedding dimension of each token in transformer part.
        ...
        # if num_encoders > 1:
        self._view_attention = TF_MultiHeadAttention(8, self._token_embedding_size, 128, 256)
        ...
        if self.using_proprioception:
            self._transformer = Transformer(
                ...
                input_token_emb_dim=self._token_embedding_size + 8,
        ...
        self._tokens_per_context_image = self._image_tokenizers["0"].tokens_per_context_image
        ...
        self._loss_object = nn.CrossEntropyLoss(reduction="mean", weight=self.loss_weight)
    ...
    def _generate_masks(self):
        ...
        action_mask = torch.from_numpy(np.ndarray(shape=(self._all_num_tokens, self._all_num_tokens), dtype=int))
    ...
    def forward(
        ...
        context_image_tokens, action_tokens, attention_mask = self._get_tokens_and_mask(observations, network_state)
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            action_t = torch.minimum(seq_idx, torch.tensor(self._time_sequence_length - 1))
            ...
            start_index = transformer_shift + self._tokens_per_context_image + action_t * (self._single_time_step_num_tokens)
            ...
            for k in range(self._tokens_per_action):
                ...
                action_tokens = action_tokens.view(b, -1)
                ...
                # [b, t * self._tokens_per_action] -> [b, t, self._tokens_per_action]
                action_tokens = action_tokens.view(b, t, self._tokens_per_action)
            ...
            # action_tokens, (b, t, self._tokens_per_action)
            # action_loss: (b, t)
            action_loss = self._loss_object(action_logits_for_training, action_tokens)  # (b, t, self._tokens_per_action)
            ...
            self._aux_info.update(
                {
                    # "action_predictions": torch.argmax(action_logits_for_training, dim=-1),
                    "action_predictions": predicted_tokens_for_output.detach(),
    ...
    def _assemble_input_token_sequence(
        ...
        if self.using_proprioception:
            input_token_sequence = input_token_sequence.view(
                batch_size, -1, self._token_embedding_size + 8
            )
        else:
            input_token_sequence = input_token_sequence.view(
    ...
    def _tokenize_images(self, observations, network_state):
        ...
        context_image_tokens = self._merge_multi_view_tokens(context_image_tokens)
        # num_tokens = context_image_tokens.shape[2]
        if self.using_proprioception:
            context_pos_orn = torch.cat((observations["position"], observations["orientation"], observations['gripper_status']),
                                        dim=-1)
        ...
        if self.using_proprioception:
            context_image_tokens = self._merge_pose_into_tokens(context_image_tokens, context_pos_orn)
    ...
    def _merge_multi_view_tokens(self, tokens):
        num_views = len(tokens)
        # if num_views == 1:
        #     return tokens
        B, T, num_tokens, token_embs = tokens[0].shape

        multi_view_img_token = torch.stack(tokens, dim=2)
        multi_view_img_token = multi_view_img_token.reshape(B * T, num_views * num_tokens, token_embs)

        attention_mask = torch.tril(torch.ones(num_tokens, num_tokens, dtype=torch.uint8)).repeat([1, num_views])
        # attention_mask = torch.diag(torch.full([num_tokens], 1, dtype=torch.uint8)).repeat([1, num_views])

        multi_view_img_token = self._view_attention(tokens[0].reshape(B * T, num_tokens, token_embs),
                                                    multi_view_img_token, multi_view_img_token, attention_mask)
        multi_view_img_token = multi_view_img_token.reshape(B, T, num_tokens, token_embs)
        return multi_view_img_token

    def _merge_pose_into_tokens(self, context_image_tokens, context_pos_orn):
        num_tokens = context_image_tokens.shape[2]
        context_pos_orn_rep = context_pos_orn.unsqueeze(2).repeat(1, 1, num_tokens, 1)
        return torch.cat((context_image_tokens, context_pos_orn_rep), dim=-1)
    ...
    def _tokenize_actions(self, observations, network_state):
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            if self._actions is None:  # When there is no action that will be tokenized to begin with, we create zero tensor.
                b, t = self._get_batch_size_and_seq_len(network_state)
                action_tokens = torch.zeros(shape=[b, t, self._tokens_per_action], dtype=torch.int32)
            else:
