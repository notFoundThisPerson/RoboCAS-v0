...
def maybe_restore_with_film(
    *args, weights="imagenet", include_top=True, include_film=False, **kwargs
):
    assert (
        weights == None or weights == "imagenet"
...
def EfficientNetB3(
    weights="imagenet", include_top=True, classes=1000, include_film=False, **kwargs
):
...
class ILSVRCPredictor:
    def __init__(self, top=5):
        ...
        image_json_path = os.path.join(
            os.path.dirname(__file__), "efficientnet_checkpoints/imagenet_classes.json"
        )
    ...
    def predict_topk(self, out):
        assert (
            out.shape[0] == 1
        ), "Only accept tonsor with batch size 1 to simplify implementation"
